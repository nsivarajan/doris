...
void process_compaction_job(MetaServiceCode& code, std::string& msg, std::stringstream& ss,
    ...
    // Read input rowsets once. For TT tables this feeds the checkpoint write (Phase 1);
    // for all tables it feeds the cleanup scan (Phase 2). Avoids duplicate FDB reads.
    ...
    if (tt_retention_days > 0 || !is_versioned_read) {
        ...
        if (snap_err != TxnErrorCode::TXN_OK) {
            LOG(WARNING) << "failed to read compaction input rowset metas, tablet_id=" << tablet_id
                         << " err=" << snap_err;
        }
    }
    ...
    // Write compaction checkpoint for time-travel enabled tables.
    // Must commit in a separate transaction BEFORE the cleanup transaction (Phase 2)
    // so that if Phase 2 commits, the checkpoint is guaranteed to exist.
    // Failure is non-fatal — compaction proceeds, but historical reads at the affected
    // version range will fall back to the per-version key path if available.
    if (tt_retention_days > 0) {
        if (input_rowsets_snapshot.empty()) {
            ...
            std::unique_ptr<Transaction> phase1_txn;
            TxnErrorCode p1_err = txn_kv->create_txn(&phase1_txn);
            if (p1_err == TxnErrorCode::TXN_OK) {
                ...
                if (checkpoint.SerializeToString(&ck_val)) {
                    phase1_txn->put(ck_key, ck_val);
                    p1_err = phase1_txn->commit();
                } else {
            ...
            if (p1_err != TxnErrorCode::TXN_OK) {
                LOG(WARNING) << "TT checkpoint commit failed (non-fatal), tablet_id=" << tablet_id
                             << " err=" << p1_err;
                // Fall through — compaction cleanup (Phase 2) still runs.
            } else {
    ...
    if (!is_versioned_read) {
        // Reuse the snapshot read above if available (avoids a second FDB scan).
        if (!input_rowsets_snapshot.empty()) {
            for (auto&& rs : input_rowsets_snapshot) {
        ...
        // is_versioned_read uses the pre-fetched snapshot directly.
        if (!input_rowsets_snapshot.empty()) {
            num_rowsets = static_cast<int>(input_rowsets_snapshot.size());
            for (auto&& rs : input_rowsets_snapshot) {
                handle_compaction_input_rowset_meta(std::move(rs));
