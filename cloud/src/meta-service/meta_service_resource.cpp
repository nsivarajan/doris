...
void MetaServiceImpl::alter_obj_store_info(google::protobuf::RpcController* controller,
    ...
    case AlterObjStoreInfoRequest::ADD_OBJ_INFO: {
        ...
        // ATTN: prefix may be empty
        // For INSTANCE_PROFILE mode, credentials come from instance metadata — no ak/sk or role_arn required.
        bool obj_is_instance_profile = obj.has_cred_provider_type() &&
                                       obj.cred_provider_type() == CredProviderTypePB::INSTANCE_PROFILE;
        if ((!obj_is_instance_profile && (ak.empty() || sk.empty()) && role_arn.empty())
