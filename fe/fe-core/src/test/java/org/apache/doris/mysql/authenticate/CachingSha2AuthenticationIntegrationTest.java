...
public class CachingSha2AuthenticationIntegrationTest {
    ...
    void setUp() throws Exception {
        ...
        // Setup mock context
        when(mockContext.getConnectionId()).thenReturn(12345);
        when(mockContext.getMysqlChannel()).thenReturn(mockChannel);
        
        // Setup mock channel
        when(mockChannel.isSslMode()).thenReturn(false); // Default to non-SSL for RSA testing
    }
    ...
    void testEnhancedHandshakePacketCreation() {
        ...
        assertEquals(MysqlEnhancedHandshakePacket.CACHING_SHA2_PASSWORD_PLUGIN, 
                    handshake.getSelectedAuthPlugin());
        assertTrue(handshake.isEnhancedAuthEnabled());
        
        // Test plugin compatibility check
        assertTrue(handshake.checkAuthPluginSameAsDoris(
                MysqlEnhancedHandshakePacket.CACHING_SHA2_PASSWORD_PLUGIN));
        assertFalse(handshake.checkAuthPluginSameAsDoris(
    ...
    void testEnhancedHandshakePacketWithDisabledFeature() {
        ...
        assertNotNull(handshake);
        assertEquals(MysqlEnhancedHandshakePacket.MYSQL_NATIVE_PASSWORD_PLUGIN, 
                    handshake.getSelectedAuthPlugin());
        assertFalse(handshake.isEnhancedAuthEnabled());
    }
    ...
    void testAuthenticationStateMachineFlow() throws Exception {
        ...
        // Initial state
        assertEquals(AuthenticationStateMachine.State.INITIAL, stateMachine.getCurrentState());
        assertFalse(stateMachine.isAuthenticationComplete());
        assertFalse(stateMachine.isAuthenticationSuccessful());
        
        // Send handshake
        assertTrue(stateMachine.processEvent(AuthenticationStateMachine.Event.SEND_HANDSHAKE));
        assertEquals(AuthenticationStateMachine.State.HANDSHAKE_SENT, stateMachine.getCurrentState());
        
        // Receive auth packet
        assertTrue(stateMachine.processEvent(AuthenticationStateMachine.Event.RECEIVE_AUTH_PACKET));
        assertEquals(AuthenticationStateMachine.State.AUTH_PACKET_RECEIVED, stateMachine.getCurrentState());
        
        // Authentication success
        assertTrue(stateMachine.processEvent(AuthenticationStateMachine.Event.AUTHENTICATE_SUCCESS));
        assertEquals(AuthenticationStateMachine.State.AUTHENTICATED, stateMachine.getCurrentState());
        assertTrue(stateMachine.isAuthenticationComplete());
        assertTrue(stateMachine.isAuthenticationSuccessful());
    }
    ...
    void testAuthenticationStateMachinePluginSwitch() throws Exception {
        ...
        assertTrue(stateMachine.processEvent(AuthenticationStateMachine.Event.PLUGIN_MISMATCH));
        assertEquals(AuthenticationStateMachine.State.PLUGIN_SWITCH_REQUIRED, stateMachine.getCurrentState());
        
        // Send plugin switch
        assertTrue(stateMachine.processEvent(AuthenticationStateMachine.Event.SEND_PLUGIN_SWITCH));
        assertEquals(AuthenticationStateMachine.State.PLUGIN_SWITCH_SENT, stateMachine.getCurrentState());
        
        // Receive plugin response and authenticate
        assertTrue(stateMachine.processEvent(AuthenticationStateMachine.Event.RECEIVE_PLUGIN_RESPONSE));
        assertEquals(AuthenticationStateMachine.State.AUTHENTICATED, stateMachine.getCurrentState());
    }
    ...
    void testSecurityTransportDecisionWithSSL() {
        when(mockChannel.isSslMode()).thenReturn(true);
        ...
        assertEquals(SecurityTransportDecision.TransportMethod.SSL_ENCRYPTED, decision.getMethod());
        assertTrue(decision.isSecure());
        assertFalse(decision.requiresRSAKey());
        assertTrue(decision.getReason().contains("SSL"));
    }
    
    @Test
    @DisplayName("Security transport decision with RSA fallback")
    void testSecurityTransportDecisionWithRSAFallback() {
        when(mockChannel.isSslMode()).thenReturn(false);
        ...
        if (decision.getMethod() == SecurityTransportDecision.TransportMethod.RSA_ENCRYPTED) {
            assertTrue(decision.isSecure());
            assertTrue(decision.requiresRSAKey());
        }
    }
    ...
    void testSecurityTransportDecisionWithDisabledFeature() {
        ...
        assertNotNull(decision);
        assertEquals(SecurityTransportDecision.TransportMethod.INSECURE_FALLBACK, decision.getMethod());
        assertFalse(decision.isSecure());
        assertFalse(decision.requiresRSAKey());
        assertTrue(decision.getReason().contains("disabled"));
    }
    ...
    void testEnhancedAuthenticatorWithCachingSha2Password() throws Exception {
        ...
        when(request.getUserName()).thenReturn("test_user");
        when(request.getRemoteIp()).thenReturn("127.0.0.1");
        when(request.getPassword()).thenReturn((Password) password);
    ...
    void testEnhancedAuthenticatorPasswordResolverSelection() {
        ...
        assertTrue(enabledAuthenticator.getPasswordResolver() instanceof CachingSha2PasswordResolver);
        ...
        assertNotNull(disabledAuthenticator.getPasswordResolver());
    ...
    void testEnhancedAuthenticatorStatistics() {
        ...
        assertEquals(0, initialMetrics.getTotalCount());
        assertEquals(0, initialMetrics.getCachingSha2Count());
        assertEquals(0, initialMetrics.getNativePasswordCount());
        assertEquals(0, initialMetrics.getSuccessCount());
        assertEquals(0, initialMetrics.getFailureCount());
        assertEquals(0.0, initialMetrics.getSuccessRate());
        
        // Statistics should be updated after authentication attempts
        String stats = authenticator.getStatistics();
        assertNotNull(stats);
        assertTrue(stats.contains("EnhancedAuthenticator"));
        assertTrue(stats.contains("enhanced_enabled=true"));
    }
    ...
    void testSecurityTransportStatistics() {
        ...
        assertEquals(0, initialMetrics.getTotalDecisions());
        ...
        SecurityTransportDecision.TransportMetrics updatedMetrics = 
                SecurityTransportDecision.getTransportMetrics();
        assertEquals(2, updatedMetrics.getTotalDecisions());
        
        String stats = SecurityTransportDecision.getTransportStatistics();
        assertNotNull(stats);
        assertTrue(stats.contains("TransportStats"));
    ...
    void testCompleteAuthenticationFlowIntegration() throws Exception {
        ...
        assertTrue(authenticator.getPasswordResolver() instanceof CachingSha2PasswordResolver);
        
        // 5. Verify components work together
        assertTrue(SecurityTransportDecision.isSecureTransport(
                SecurityTransportDecision.TransportMethod.SSL_ENCRYPTED));
        assertTrue(SecurityTransportDecision.isSecureTransport(
                SecurityTransportDecision.TransportMethod.RSA_ENCRYPTED));
        assertFalse(SecurityTransportDecision.isSecureTransport(
    ...
    void testPasswordCacheIntegration() {
        ...
        assertFalse(cache.isPasswordCached(username, passwordHash));
        
        // Cache password
        cache.cachePassword(username, passwordHash);
        assertTrue(cache.isPasswordCached(username, passwordHash));
        
        // Different password should not be cached
        byte[] differentHash = "different_password_hash".getBytes();
        assertFalse(cache.isPasswordCached(username, differentHash));
