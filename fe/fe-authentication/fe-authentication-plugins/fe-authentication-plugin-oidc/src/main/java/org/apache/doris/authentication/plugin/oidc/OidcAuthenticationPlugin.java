...
public class OidcAuthenticationPlugin implements AuthenticationPlugin {
    ...
    public boolean requiresClearPassword() {
        // When this plugin is the primary authenticator (authentication_type=oidc),
        // instructs AuthenticationPluginAuthenticator to use ClearPasswordResolver,
        // which sends MysqlAuthSwitchPacket to the client. Any standard MySQL connector
        // (PyMySQL, JDBC, mysql-connector-python) responds with raw text automatically —
        // no --authentication_plugin flag or special driver config needed.
        return true;
    }

    @Override
    public boolean supports(AuthenticationRequest request) {
        String credType = request.getCredentialType();

        // Path 1: authentication_openid_connect_client MySQL plugin
        // MysqlAuthPacketCredentialExtractor already set credentialType = OAUTH_TOKEN.
        if (CredentialType.OAUTH_TOKEN.equalsIgnoreCase(credType)) {
            return supportsCredential(request.getCredential());
        }

        // Path 2: chain's ClearPasswordResolver re-challenged the client after primary auth
        // failed (canReuseRequestForAuthenticationChain returns false for native password,
        // triggering a second AuthSwitch that any standard MySQL connector responds to).
        // Also covers authentication_type=oidc primary path where requiresClearPassword()=true
        // makes Doris send AuthSwitch on first contact.
        // SSL is already enforced by isOidcAuthenticateRequest() in AuthenticatorManager
        // before this code runs — no security regression.
        if (CredentialType.CLEAR_TEXT_PASSWORD.equalsIgnoreCase(credType)) {
            byte[] cred = request.getCredential();
            if (cred == null || cred.length < 3) {
                return false;
            }
            // Only claim this request if the clear text looks like a JWT.
            // Plain passwords (admin/root) must pass through to the default authenticator.
            return looksLikeJwt(new String(cred, StandardCharsets.UTF_8).trim())
                    && supportsCredential(cred);
        }
    ...
    /**
     * Returns true if the string looks like a compact JWT (starts with "eyJ", exactly two dots).
     * Used to distinguish JWT tokens from plain passwords in the CLEAR_TEXT_PASSWORD path.
     */
    ...
    /**
     * Checks if the credential is accepted by this integration.
     * When no issuer is configured, all JWT credentials are accepted.
     * When an issuer is configured, the JWT's iss claim must match.
     */
    private boolean supportsCredential(byte[] cred) {
