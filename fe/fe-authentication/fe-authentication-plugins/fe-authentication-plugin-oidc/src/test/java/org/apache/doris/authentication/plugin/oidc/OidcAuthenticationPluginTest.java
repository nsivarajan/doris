...
class OidcAuthenticationPluginTest {
    ...
    private static AuthenticationRequest clearTextRequest(String username, String password) {
        return AuthenticationRequest.builder()
                .username(username)
    ...
    private AuthenticationIntegration buildIntegration(String issuer) {
        return AuthenticationIntegration.builder()
                .name("idms_with_issuer").type("oidc")
                .property(OidcAuthenticationPlugin.PROP_USERINFO_URL,
                        "https://idmsac.apple.com/auth/oauth2/userinfo")
                .property(OidcAuthenticationPlugin.PROP_CLIENT_ID, "66rrcbuuyvp3f6fszcv5mybsr3vai9")
                .property(OidcAuthenticationPlugin.PROP_ISSUER, issuer)
                .build();
    }
    ...
    // ── requiresClearPassword ─────────────────────────────────────────────────

    @Test
    void testRequiresClearPassword_returnsTrue() {
        // Ensures AuthenticationPluginAuthenticator uses ClearPasswordResolver
        // when this plugin is the primary authenticator, triggering AuthSwitch
        // so any standard MySQL connector sends raw text without special flags.
        Assertions.assertTrue(plugin.requiresClearPassword());
    }

    // ── supports: CLEAR_TEXT_PASSWORD path (standard connectors) ─────────────

    @Test
    void testSupports_clearTextJwt_noIssuer_accepted() throws AuthenticationException {
        plugin.initialize(integration); // no issuer configured
        AuthenticationRequest req = clearTextRequest("2701084255",
                fakeJwt("{\"sub\":\"2701084255\",\"iss\":\"https://any.com\"}"));
        Assertions.assertTrue(plugin.supports(req));
    }

    @Test
    void testSupports_clearTextPlainPassword_rejected() throws AuthenticationException {
        plugin.initialize(integration);
        AuthenticationRequest req = clearTextRequest("admin", "plainpassword");
        // Plain password doesn't look like a JWT — must not be claimed by OIDC plugin
        Assertions.assertFalse(plugin.supports(req));
    }

    @Test
    void testSupports_clearTextJwt_issuerMatches() throws AuthenticationException {
        AuthenticationIntegration withIssuer = buildIntegration("https://idmsac.apple.com");
        plugin.initialize(withIssuer);

        AuthenticationRequest req = clearTextRequest("2701084255",
                fakeJwt("{\"sub\":\"2701084255\",\"iss\":\"https://idmsac.apple.com\"}"));
        Assertions.assertTrue(plugin.supports(req));
    }

    @Test
    void testSupports_clearTextJwt_issuerMismatch_rejected() throws AuthenticationException {
        AuthenticationIntegration withIssuer = buildIntegration("https://idmsac.apple.com");
        plugin.initialize(withIssuer);

        AuthenticationRequest req = clearTextRequest("2701084255",
                fakeJwt("{\"sub\":\"2701084255\",\"iss\":\"https://idmsac-cn.apple.com\"}"));
        Assertions.assertFalse(plugin.supports(req));
    }

    @Test
    void testAuthenticate_withClearTextCredentialType_works() throws AuthenticationException {
        long futureExp = Instant.now().getEpochSecond() + 3600;
        plugin.returnUserinfo(userinfoFor("2701084255", futureExp));

        // Simulates what the chain's ClearPasswordResolver delivers after AuthSwitch
        AuthenticationRequest req = clearTextRequest("2701084255",
                fakeJwt("{\"sub\":\"2701084255\"}"));
    ...
    @Test
    void testLooksLikeJwt_validJwt() {
        Assertions.assertTrue(OidcAuthenticationPlugin.looksLikeJwt(
                fakeJwt("{\"sub\":\"123\"}")));
    }

    @Test
    void testLooksLikeJwt_plainPassword() {
        Assertions.assertFalse(OidcAuthenticationPlugin.looksLikeJwt("mypassword"));
    }

    @Test
    void testLooksLikeJwt_wrongPrefix() {
        Assertions.assertFalse(OidcAuthenticationPlugin.looksLikeJwt("abc.def.ghi"));
    }

    @Test
    void testLooksLikeJwt_null() {
        Assertions.assertFalse(OidcAuthenticationPlugin.looksLikeJwt(null));
    }
    ...
    void testAuthenticate_expiredToken_fromUserinfo() throws AuthenticationException {
        ...
        Assertions.assertFalse(result.isSuccess());
        Assertions.assertNotNull(result.getException().getMessage());
        Assertions.assertTrue(result.getException().getMessage().contains("expired"),
                "Expected 'expired' in: " + result.getException().getMessage());
    }
    ...
    void testAuthenticate_dsidMismatch_fromUserinfo() throws AuthenticationException {
        ...
        Assertions.assertFalse(result.isSuccess());
        Assertions.assertNotNull(result.getException().getMessage());
        Assertions.assertTrue(result.getException().getMessage().contains("subject does not match"),
                "Expected 'subject does not match' in: " + result.getException().getMessage());
    }
    ...
    void testAuthenticate_groupsExtractedFromExtendedClaims() throws AuthenticationException {
        ...
        Assertions.assertTrue(groups.contains("13455565"));
        Assertions.assertTrue(groups.contains("8675466"));
    }
    ...
    void testAuthenticate_noGroupsReturnedWhenExtendedClaimsMissing() throws AuthenticationException {
        ...
        Assertions.assertTrue(result.isSuccess());
        Assertions.assertTrue(result.getPrincipal().getExternalGroups().isEmpty());
    }
    ...
    void testSupports_issuerMatches_acceptsToken() throws AuthenticationException {
        ...
        Assertions.assertTrue(plugin.supports(oauthRequest("2701084255", jwt)));
    }
    ...
    void testSupports_issuerMismatch_returnsFalseCleanly() throws AuthenticationException {
        ...
        Assertions.assertFalse(plugin.supports(oauthRequest("2701084255", chinaJwt)));
    }
    ...
    void testSupports_noIssuerConfigured_acceptsAllJwt() throws AuthenticationException {
        ...
        Assertions.assertTrue(plugin.supports(oauthRequest("2701084255", anyJwt)));
    }
    ...
    void testValidate_missingUserinfoUrl() {
        AuthenticationIntegration bad = AuthenticationIntegration.builder()
                ...
                .build();
        Assertions.assertThrows(AuthenticationException.class, () -> plugin.validate(bad));
    }
