...
int ECSMetadataCredentialsProvider::_get_role_name(std::string& role_name) {
    ...
    if (newline_pos != std::string::npos) {
        ...
        LOG(WARNING) << "Multiple RAM roles found on this ECS instance; using first: " << role_name
                     << ". All bound roles: " << all_roles
                     << ". Bind a single RAM role to this instance to avoid ambiguity.";
    }
...
OSSSTSCredentialProvider::OSSSTSCredentialProvider(const std::string& role_arn,
    ...
    LOG(INFO) << "OSSSTSCredentialProvider: role_arn=" << _role_arn << ", region=" << _region
              << ", external_id="
              << (_external_id.empty() ? "(none)" : mask_credential(_external_id));
}
...
int OSSSTSCredentialProvider::_fetch_credentials_from_sts(
    ...
    try {
        ...
        // Use override endpoint for non-standard regions; default: "sts.<region>.aliyuncs.com".
        ...
        // Include hostname in session name for ActionTrail audit traceability.
        // STS session names allow [a-zA-Z0-9._-], max 32 chars.
        ...
        if (session_name.size() < 2) {
            session_name = "doris-oss-session";
        }
        ...
        if (!_ca_cert_path.empty()) {
            ...
            LOG(WARNING) << "OSSSTSCredentialProvider: no CA cert path, skipping SSL verification. "
                            "Set ca_cert_file_paths in be.conf.";
...
AlibabaCloud::OSS::Credentials OSSDefaultCredentialsProvider::getCredentials() {
    ...
    try {
        ...
        if (ak.empty() || sk.empty()) {
            ...
            LOG(ERROR) << "Default provider chain returned empty credentials and no cached "
                          "fallback available";
            return AlibabaCloud::OSS::Credentials("", "", "");
        }

        auto new_credentials =
                token.empty() ? std::make_unique<AlibabaCloud::OSS::Credentials>(ak, sk)
        ...
        LOG(ERROR) << "Failed to get OSS credentials and no cached fallback available: "
                   << e.what();
        return AlibabaCloud::OSS::Credentials("", "", "");
    }
}
