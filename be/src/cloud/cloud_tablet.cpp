...
Status CloudTablet::capture_rs_readers_with_tt_rowsets(
    ...
    // Build a version path augmented with TT extra rowsets (pre-compaction originals).
    // The shared tablet state is never modified — only a shared read lock is held.
    std::shared_lock rlock(_meta_lock);
