...
class OssObjStorageClient final : public ObjStorageClient {
public:
#ifdef USE_OSS
    OssObjStorageClient(std::shared_ptr<AlibabaCloud::OSS::OssClient> client,
                        const std::string& bucket);
#else
    OssObjStorageClient() = delete;
#endif
    ~OssObjStorageClient() override = default;

    // Implement all ObjStorageClient virtual methods
    ...
    ObjectStorageUploadResponse upload_part(const ObjectStoragePathOptions& opts,
                                           std::string_view stream, int partNum) override;
